...
wi = st.number_input("wi [%]", min_value=0.0, max_value=1000.0, step=0.1, value=20.0)
LL = st.number_input("LL [%]", min_value=0.0, max_value=1000.0, step=0.1, value=102.0)
Sand = st.number_input("Sand [%]", min_value=0.0, max_value=1000.0, step=0.1, value=2.0)
PI = st.number_input("PI [%]", min_value=0.0, max_value=1000.0, step=0.1, value=82.0)
C = st.number_input("C [%]", min_value=0.0, max_value=1000.0, step=0.1, value=85.0)
A = st.number_input("A", min_value=0.0, max_value=1000.0, step=0.1, value=0.96)
LP = st.number_input("LP [%]", min_value=0.0, max_value=1000.0, step=0.1, value=20.0)
...
if st.button("Predict Swelling Pressure"):
    ...
    if None in [A, Sand, C, LL, LP, PI, gamma_d, wi]:
        ...
        try:
            ...
            st.success(f"Predicted Swelling Pressure (Pr [MPa]): {prediction[0]:.4f}")
        except Exception as e:
